...
# -----------------------------------------------------------------------------
# 1. 데이터 로드 (캐시 문제 해결 버전)
# -----------------------------------------------------------------------------
# ttl=0 : 캐시를 저장하지 않고 매번 새로 읽음 (파일 갱신 문제 해결)
@st.cache_data(ttl=0)
    ...
    try:
        ...
        for req_sheet in required:
            if req_sheet in normalized_names:
                ...
                st.error(f"❌ 엑셀 파일에 **'{req_sheet}'** 시트가 없습니다! (현재 시트 목록: {sheet_names})")
                return None
...
# 2. 업로드 없으면 기본 파일 로드 시도
else:
    try:
        data = load_data(default_file)
        if data:
            st.sidebar.info(f"기본 파일 로드됨: {default_file}")
    except:
        pass
